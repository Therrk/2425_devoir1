...
float deriv_eval(float cmv, float y[], float e){
  float left_out, left_midle, right_midle, right_out, bottom;
  left_out = 0; left_midle = 0; right_midle = 0; right_out = 0; bottom = 0;
  int i;
  for (i = 0; i < 10; i++) {
	  left_out = left_out + pow(y[i],cmv);
	  left_midle = left_midle + cmv*pow(y[i],cmv-1)*log(y[i]);
	  right_midle = right_midle + cmv*pow(y[i],cmv-1);
	  right_out = right_out + pow(y[i],cmv-1)*log(y[i]);
	  bottom = bottom + pow(y[i], 2*cmv);
  }
  return 1/pow(cmv,2)+(left_out*left_midle-right_midle*right_out)/bottom;
}
